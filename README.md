...
### **How It Works**  
1. **Speech & AI Interaction**: 
The system uses Azure OpenAI GPT-3.5 Turbo, integrated with the Semantic Kernel Planner, to handle intelligent conversations and order management.
2. **Database Management**: 
Order details are stored and retrieved using Cosmos DB, providing a NoSQL solution for efficient and scalable data storage.
3. **Voice Responses**: 
The system integrates Azure AI Speech Service – Speech Synthesis, enabling real-time voice interactions with customers during the ordering process.
4. **Custom Plugins**: 
C# plugins are used to handle database CRUD operations, ensuring seamless transaction processing and integration within the system.
